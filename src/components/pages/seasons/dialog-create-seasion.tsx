...
const DialogCreateSeason = ({
  ...
  const { register, handleSubmit, setValue, watch,reset } = useForm<CreateSeasonRequest>({
    defaultValues: {
      gameId: 0,
      name: "",
      description: "",
    ...
    <Dialog open={isCreateSeasonOpen} onOpenChange={setIsCreateSeasonOpen}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>Fill in the information to create a new season.</DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4 py-4">
          ...
          <div className="flex items-center gap-4">
            <Label htmlFor="collectionNFTName" className=" min-w-[120px]">NFT Collection Name</Label>
            <Input id="collectionNFTName" {...register("collectionNFTName")} placeholder="Enter NFT Collection Name" />
          </div>

          <div className="flex items-center gap-4">
            <Label htmlFor="collectionNFTSymbol" className=" min-w-[120px]">NFT Collection Symbol</Label>
            <Input id="collectionNFTSymbol" {...register("collectionNFTSymbol")} placeholder="Enter NFT Collection Symbol" />
          </div>

          <div className="flex items-center gap-4">
            <Label htmlFor="active" className="text-right min-w-[120px]">NFT required</Label>
            <Select
              value={watch("requireNFT")?.toString()}
              onValueChange={(value) => setValue("requireNFT", value === "true")}
            >
              <SelectTrigger className="w-full">
          ...
          <div className="flex items-center gap-4">
            <Label htmlFor="startDate" className="text-right min-w-[120px]">Start Date</Label>
            <DatePicker
           ...
           <div className="flex items-center gap-4">
            <Label htmlFor="endDate" className="text-right min-w-[120px]">End Date</Label>
            <DatePicker
              value={watch("endDate")}
          ...
          <DialogFooter>
            <Button variant="outline" type="button" onClick={() => setIsCreateSeasonOpen(false)}>Cancel</Button>
